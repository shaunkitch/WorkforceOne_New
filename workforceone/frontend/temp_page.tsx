...
export default function TeamsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {activeTab === 'invitations' && (canManageTeams() || canChangeUserRoles()) && (
        <div className="space-y-6">
          ...
          <Card>
            ...
            <CardContent>
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <h4 className="font-medium text-blue-900 mb-2">How to Invite Members</h4>
                <ul className="text-sm text-blue-800 space-y-1">
                  <li>• Click "Send Invitations" to invite multiple team members at once</li>
                  <li>• Assign roles (Member, Manager, Admin) and departments</li>
                  <li>• Invitations expire after 7 days but can be resent</li>
                  <li>• Track invitation status in the table below</li>
                </ul>
              </div>
            </CardContent>
          </Card>
              ...
              <CardTitle className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-4">
                  <span className="text-sm text-gray-500 font-normal">
                    {pendingInvitations.filter(inv => inv.status === 'pending').length} active, 
                    {' '}{pendingInvitations.filter(inv => inv.status === 'expired').length} expired
                  </span>
                  <Button
                    size="sm"
                    variant="outline"
              ...
              <div className="space-y-3">
                {pendingInvitations.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <p className="text-gray-500 mb-4">Start by sending invitations to new team members</p>
                    <Button 
                      onClick={() => setShowInviteModal(true)}
                      className="bg-green-600 hover:bg-green-700"
                    >
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                          {pendingInvitations.map(invitation => (
                            <tr key={invitation.id} className="hover:bg-gray-50">
                              ...
                              <td className="px-6 py-4 whitespace-nowrap">
                                <div>
                                  <div className="text-sm font-medium text-gray-900">{invitation.role}</div>
                              ...
                              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                <div>
                                  <div>{format(parseISO(invitation.created_at), 'MMM d, yyyy')}</div>
                                  ...
                                  <Button
                                    ...
                                    onClick={() => {
                                      navigator.clipboard.writeText(
                                        `${window.location.origin}/signup?token=${invitation.invitation_token}`
                                      )
                    ...
                    <div className="mt-6 pt-4 border-t bg-gray-50 rounded-lg p-4">
                      <div className="flex items-center justify-between">
                        <div className="text-sm text-gray-600">
                          ...
                          <p className="text-xs mt-1">Manage multiple invitations at once</p>
                        </div>
                        <div className="flex space-x-3">
                          <Button
                            size="sm"
                            variant="outline"
                            onClick={() => {
                              const expiredInvitations = pendingInvitations.filter(inv => inv.status === 'expired')
                              if (expiredInvitations.length === 0) {
                                alert('No expired invitations to resend')
                                return
                              }
                              if (confirm('Resend ' + expiredInvitations.length + ' expired invitation(s)?')) {
                                expiredInvitations.forEach(inv => resendInvitation(inv.id))
                              }
                            }}
                            disabled={pendingInvitations.filter(inv => inv.status === 'expired').length === 0}
                          >
                            <RefreshCw className="h-4 w-4 mr-2" />
                            Resend All Expired ({pendingInvitations.filter(inv => inv.status === 'expired').length})
                          </Button>
                          <Button
                            size="sm"
                            variant="outline"
                            onClick={() => {
                              ...
                              if (pendingCount === 0) {
                                alert('No pending invitations to cancel')
                                return
                              }
                              if (confirm('Cancel all ' + pendingCount + ' pending invitation(s)?')) {
                                pendingInvitations
                                  .filter(inv => inv.status === 'pending')
                                  .forEach(inv => cancelInvitation(inv.id))
                              }
                            }}
                            disabled={pendingInvitations.filter(inv => inv.status === 'pending').length === 0}
                            className="text-red-600 hover:text-red-700 border-red-200 hover:bg-red-50"
                          >
                            <XCircle className="h-4 w-4 mr-2" />
                            Cancel All Pending ({pendingInvitations.filter(inv => inv.status === 'pending').length})
                          </Button>
                        </div>
                      </div>
                    </div>
