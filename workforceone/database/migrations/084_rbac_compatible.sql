-- ============================================
-- COMPATIBLE RBAC SYSTEM FOR EXISTING SCHEMA
-- Migration: 084_rbac_compatible.sql
...
CREATE POLICY "Allow admins to manage roles" ON roles FOR ALL TO authenticated 
    USING (EXISTS(
...
CREATE POLICY "Allow viewing own organization features" ON organization_features FOR SELECT TO authenticated 
    USING (EXISTS(
        SELECT 1 FROM profiles 
        WHERE profiles.id = auth.uid() 
...
CREATE POLICY "Allow admins to manage organization features" ON organization_features FOR ALL TO authenticated 
    USING (EXISTS(
        SELECT 1 FROM profiles 
        WHERE profiles.id = auth.uid() 
        AND profiles.organization_id = organization_features.organization_id
...
COMMENT ON FUNCTION has_feature_access IS 'Check if user has access to a specific feature (works with legacy feature_flags)';
COMMENT ON FUNCTION has_permission IS 'Check if user has a specific permission based on their role';