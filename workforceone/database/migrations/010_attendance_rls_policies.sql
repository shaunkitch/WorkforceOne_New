-- Attendance table Row Level Security policies
-- Ensures proper data segregation for attendance records
...
-- Users can see their own records, managers/admins can see all records in their organization
CREATE POLICY "Users can view attendance in their organization" ON public.attendance
FOR SELECT
TO authenticated
USING (
  ...
  user_id = auth.uid()
  OR
  -- Managers and admins can see all attendance records in their organization
  EXISTS (
    SELECT 1 FROM public.profiles 
    WHERE profiles.id = auth.uid() 
    AND profiles.organization_id = attendance.organization_id
    AND profiles.role IN ('admin', 'manager')
  )
);

-- Policy for inserting attendance records
-- Users can only create attendance records for themselves in their organization
...
-- Users can update their own records (for check-out, notes, etc.)
-- Managers/admins can update any attendance records in their organization
...
-- Only admins can delete attendance records in their organization
CREATE POLICY "Users can delete attendance in their organization" ON public.attendance
FOR DELETE
TO authenticated
USING (
  EXISTS (
...
-- =============================================================================
-- ATTENDANCE REMINDERS TABLE (for scheduled reminders)
-- =============================================================================

-- Create attendance_reminders table for automated reminders
...
-- Update trigger for attendance_reminders
