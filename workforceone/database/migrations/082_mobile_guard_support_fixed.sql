-- Mobile Guard Support Tables - Fixed Version
-- Simplified tables for mobile app functionality with consistent UUID types

-- Ensure patrol_sessions table exists (fixed version with UUID)
...
-- Ensure checkpoint_visits table exists (fixed version with UUID foreign key)
...
-- Create backup_requests table for mobile app (UUID primary key)
...
-- Create emergency_alerts table for mobile app (UUID primary key)
...
-- Create daily_reports table for mobile app (UUID primary key)
...
DROP POLICY IF EXISTS "Users can manage own patrol sessions" ON patrol_sessions;
CREATE POLICY "Users can manage own patrol sessions" ON patrol_sessions
    FOR ALL USING (guard_id = auth.uid());

DROP POLICY IF EXISTS "Users can manage own checkpoint visits" ON checkpoint_visits;
...
DROP POLICY IF EXISTS "Users can manage own backup requests" ON backup_requests;
...
DROP POLICY IF EXISTS "Users can manage own daily reports" ON daily_reports;
...
DROP POLICY IF EXISTS "Guard managers can view all patrol data" ON patrol_sessions;
...
DROP POLICY IF EXISTS "Guard managers can view all checkpoint visits" ON checkpoint_visits;
...
DROP POLICY IF EXISTS "Guard managers can view all backup requests" ON backup_requests;
...
DROP POLICY IF EXISTS "Guard managers can view all emergency alerts" ON emergency_alerts;
...
DROP POLICY IF EXISTS "Guard managers can view all daily reports" ON daily_reports;
...
DROP TRIGGER IF EXISTS update_patrol_sessions_updated_at ON patrol_sessions;
CREATE TRIGGER update_patrol_sessions_updated_at BEFORE UPDATE ON patrol_sessions
    FOR EACH ROW EXECUTE PROCEDURE update_updated_at_column();

DROP TRIGGER IF EXISTS update_backup_requests_updated_at ON backup_requests;
...
-- Success message
SELECT 'Mobile Guard Support Tables created successfully with UUID consistency!' as message;