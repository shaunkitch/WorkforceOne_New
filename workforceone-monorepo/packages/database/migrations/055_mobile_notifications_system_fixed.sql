-- =============================================
-- Mobile Notifications System (Fixed Version)
-- Create comprehensive notification and messaging system
-- =============================================

-- First, check if tables already exist and clean up if needed
DO $$ 
BEGIN
    ...
    DROP FUNCTION IF EXISTS expand_form_assignments();
    DROP FUNCTION IF EXISTS notify_form_assignment();
    DROP FUNCTION IF EXISTS send_push_notification(JSONB);
...
-- Create RLS policies (drop and recreate to handle existing policies)
DO $$
BEGIN
...
-- Form assignment expansion function (using correct column names)
...
-- Trigger to send notification when form is assigned (using correct column names)
...
-- Insert default notification preferences for existing users (safe approach)
DO $$
BEGIN
    -- Only insert if the table is empty or users don't have preferences yet
    ...
    RAISE NOTICE 'Default notification preferences created successfully';
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'Error creating default preferences: %. Continuing...', SQLERRM;
END $$;
...
DO $$
BEGIN
    RAISE NOTICE '✅ Mobile notifications system migration completed successfully!';
    RAISE NOTICE '📱 New features available:';
    RAISE NOTICE '  • Push notification support';
    RAISE NOTICE '  • In-app messaging system';  
    RAISE NOTICE '  • Notification preferences';
    RAISE NOTICE '  • Form assignment expansion triggers';
END $$;