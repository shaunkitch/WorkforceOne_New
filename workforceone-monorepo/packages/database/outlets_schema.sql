...
CREATE POLICY "Organization admins can manage outlet assignments" ON outlet_users
  FOR ALL USING (((SELECT organization_id FROM profiles WHERE id = auth.uid()) = (SELECT organization_id FROM outlets WHERE id = outlet_id)) AND ((SELECT role FROM profiles WHERE id = auth.uid()) = 'admin'));
