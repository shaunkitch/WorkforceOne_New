...
export default function PatrolNavigationsPage() {
  ...
  const loadNavigations = async () => {
    try {
      ...
      setNavigations(routesWithCount);
    } catch (error) {
  ...
  const handleUpdateNavigation = async () => {
    ...
    try {
      ...
      setNavigations(routes.map(route => 
  ...
  const handleDeleteNavigation = async (routeId: string) => {
    ...
    try {
      ...
      setNavigations(routes.filter(route => route.id !== routeId));
    } catch (error) {
      console.error('Error deleting route:', error);
    }
  };

  const handleToggleActive = async (route: PatrolNavigation) => {
    try {
      const { data, error } = await supabase
        .from('patrol_routes')
  ...
  const openEditModal = (route: PatrolNavigation) => {
  ...
  const openViewModal = async (route: PatrolNavigation) => {
    try {
      const { data, error } = await supabase
        .from('patrol_routes')
        .select(`
  ...
  const handleAdhocRouteGenerated = async (adhocRoute: AdhocRoute) => {
    try {
      ...
      const { data: newNavigation, error: routeError } = await supabase
        .from('patrol_routes')
        .insert([routeData])
        .select()
        .single();
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 to-indigo-50">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Patrol Navigations</h1>
        ...
        <div className="flex space-x-2">
          ...
          <DialogContent className="sm:max-w-[425px]">
            <DialogHeader>
              <DialogTitle>Create New Patrol Navigation</DialogTitle>
              <DialogDescription>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="color">Navigation Color</Label>
                <Input
                  id="color"
                  type="color"
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Active Navigations</CardTitle>
      ...
      <Card>
        <CardHeader>
          <CardTitle>All Navigations</CardTitle>
          <CardDescription>
                    ...
                    <TableCell>
                      <div className="flex items-center space-x-2">
                        ...
                        <Button 
                          variant="ghost" 
                          size="sm"
                          onClick={() => handleDeleteNavigation(route.id)}
      ...
      <Dialog open={isEditModalOpen} onOpenChange={setIsEditModalOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>Edit Patrol Navigation</DialogTitle>
            <DialogDescription>
          ...
          <div className="p-6 space-y-4">
            <div className="space-y-2">
              <Label htmlFor="edit-name">Navigation Name</Label>
              <Input
                id="edit-name"
                value={formData.name}
                onChange={(e) => setFormData({ ...formData, name: e.target.value })}
            ...
            <div className="space-y-2">
              <Label htmlFor="edit-color">Navigation Color</Label>
              <Input
                id="edit-color"
                type="color"
                value={formData.color_code}
          ...
          {selectedRoute && (
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label className="text-sm font-medium">Navigation Name</Label>
      ...
      {/* Adhoc Navigation Generator Modal */}
      <Dialog open={isAdhocModalOpen} onOpenChange={setIsAdhocModalOpen}>
        <DialogContent className="sm:max-w-[600px]">
          <DialogHeader>
            <DialogTitle>Generate Adhoc Navigation</DialogTitle>
            <DialogDescription>
